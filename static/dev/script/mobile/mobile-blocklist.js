			...
			BODY: {
				...
				CONTENTTABLE:
					...
					'<div class="pt-grid" :style="domParams.contentTable.style" :id="domParams.contentTable.id">' +
						'<div v-for="(row,index) in rows" :ns-rowindex="index" :ns-id="row[idField]" @click.self="rowClickHandler" @dblclick="rowdbClickHandler" class="pt-block-list" :class="[{\'selected\':row.netstarSelectedFlag},{\'disabled\':row[\'NETSTAR-TRDISABLE\']},plusClass,netstarRowStateFlag(row)]">' +
        ...
        getFooterTableHtml:function(_gridConfig, _vueConfig){
			...
			$.each(footerColumns, function (key, config) {
				...
				_vueConfig.computed[key] = function () {
					...
					//转化格式到显示格式
					if(formatExpression){
						returnNumber = NetStarUtils.valueConvertManager.getMoneyStringByNumber(returnNumber,formatExpression);
					}
