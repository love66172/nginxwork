...
nsFrame.loadPageVRouter = function(url){
	...
	if($('container .processdocbasetab').length > 0){
		var templateId = $('container .processdocbasetab').attr('id');
		nsFrame.cacheUrlVRouter.pageParam[nsFrame.cacheUrlVRouter.counter] = {
			templateId:templateId,
			config:NetstarTemplate.templates.processDocBaseMobile.data[templateId].config
		};
	}
	if($('container .businessdatabasem').length > 0){
		var templateId = $('container .businessdatabasem').attr('id');
		nsFrame.cacheUrlVRouter.pageParam[nsFrame.cacheUrlVRouter.counter] = {
			templateId:templateId,
			config:NetstarTemplate.templates.businessDataBaseMobile.data[templateId].config
		};
	}
	$('container').remove();
	...
	var pushUrl = (typeof(NetstarHomePage.defaultServerUrl) == "string" ? NetstarHomePage.defaultServerUrl : getRootPath())+'/mobilehome#'+urlStr;
	...
	if(window.history && window.history.pushState){
		$(window).on('popstate',function(e){
			...
			if(isContinue){
				...
				var pushUrl = (typeof(NetstarHomePage.defaultServerUrl) == "string" ? NetstarHomePage.defaultServerUrl : getRootPath())+'/mobilehome#'+nsFrame.cacheUrlVRouter.counter;
				window.history.pushState('forward',null,pushUrl);
			}
		})
	}
