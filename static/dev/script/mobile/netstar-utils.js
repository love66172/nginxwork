 ...
 * @LastEditTime: 2019-09-11 10:43:16
 */
// "use strict";
//通用工具集合 NetStarUtils
var NetStarUtils = {};
...
NetStarUtils.getDataByFormatTreeData = function(rows,level){
	var treeRows = [];
	var expandLevel = Number(level) ? level : -1;
	function formatTreeData(array,levelIndex,isHasParent){
		if(!$.isArray(array)){return;}
		var isOpen = false;//是否显示
		var isExpand = false;//当前是父节点是否展开状态
		if(expandLevel>0){
		...
		$.ajax({
			...
			success: function(data, textStatus, request){
				...
				if(typeof(token)!='string'){
					...
					console.error(request.getAllResponseHeaders());
					return;
				}
