                ...
                for(var i=0; i<strArr.length; i++){
					if(strArr[i].indexOf('.')>-1){
						...
						if(strArr[i].match(rex2)[1]=='iconClass'){
							...
							var str = data[strArr[i].match(rex2)[1]] ? data[strArr[i].match(rex2)[1]] : '';
							if(typeof(data[strArr[i].match(rex2)[1]])=='number'){
								str = data[strArr[i].match(rex2)[1]];
							}
							if(strArr[i].match(rex2)[1] == 'workItemState'){
								switch(str){
									case 1:
										str = '未解决';
										break;
								...
								var chargeIconByField = $(listHtml).find('[ns-icon="workItemState"]').attr('ns-icon');
								var chargeIcon = {
									workItemState:{
										1:'workitemstate-icon',
										2:'workitemstate-icon',
										3:'workitemstate-icon',
										4:'workitemstate-icon',
										5:'workitemstate-icon',
									}
								};
								if(chargeIcon[chargeIconByField]){
									listHtml = listHtml.replace('ns-icon="workItemState"','ns-icon="'+chargeIcon[chargeIconByField][originalRow[chargeIconByField]]+'"');
								}
							}else if(strArr[i].match(rex2)[1] == 'urgentState'){
								switch(str){
									case 0:
										str = '普通';
										break;
								...
								var chargeIconByField = $(listHtml).find('[ns-icon="urgentState"]').attr('ns-icon');
								var chargeIcon = {
									urgentState:{
										0:'urgentstate-icon-normal',
										1:'urgentstate-icon-primary',
										2:'urgentstate-icon-super',
									}
								};
								if(chargeIcon[chargeIconByField]){
									listHtml = listHtml.replace('ns-icon="urgentState"','ns-icon="'+chargeIcon[chargeIconByField][originalRow[chargeIconByField]]+'"');
								}
							}else if(strArr[i].match(rex2)[1] == 'state'){
								switch(str){
									case 0:
										str = '未检测';
										break;
								...
								var chargeIconByField = $(listHtml).find('[ns-icon="state"]').attr('ns-icon');
								var chargeIcon = {
									state:{
										0:'input-status-jc',
										1:'input-status-jc',
								...
								if(chargeIcon[chargeIconByField]){
									listHtml = listHtml.replace('ns-icon="state"','ns-icon="'+chargeIcon[chargeIconByField][originalRow[chargeIconByField]]+'"');
								}
							}
							listHtml = listHtml.replace(strArr[i],str);
						}
					}
				}
                    ...
                    originalRows:{
                        ...
                        handler:function(newRowsData, oldRowsData){
							...
							for(var i=0; i<this.originalRows.length;i++){
								if(this.rows[i]){
									this.originalRows[i].netstarSelectedFlag = this.rows[i].netstarSelectedFlag;
								}else{
									this.originalRows[i].netstarSelectedFlag =false;
								}
							}
