 ...
 * @LastEditTime: 2019-10-23 17:31:11
 */
...
NetstarUI.resultTable.loadData = function(config){
	...
	var tableAjax = $.extend(true,{},config.tableAjax);
	if(config.tableAjax.data && !$.isEmptyObject(config.pageParam)){
		tableAjax.data = NetStarUtils.getFormatParameterJSON(config.tableAjax.data,config.pageParam);
	}
	NetStarUtils.ajax(tableAjax, function(data){
		if(data.success){
			if(debugerMode){
...
NetstarUI.resultTable.autoSubmitHandler = function(){
	...
	if(NetstarUI.resultTable.offlineState){
		...
		var ajaxConfig = $.extend(true,{},config.saveAjax);
...
NetstarUI.resultTable.multiuploadInit = function(resizeData){
				...
				btns:[
					{
						...
						handler:function(){
							...
							if(nsIndex > 0){
								...
								var ajaxConfig = $.extend(true,{},NetstarUI.resultTable.config.default.uploadSaveAjax);
								ajaxConfig.data = _postData;
								NetStarUtils.ajax(ajaxConfig,function(data){
									if(data.success){
					...
					},
					{
						...
						handler:function(){
							...
							var ajaxConfig = $.extend(true,{},NetstarUI.resultTable.config.default.uploadDelAjax);
							NetStarUtils.ajax(ajaxConfig,function(data){
								if(data.success){
	...
	function saveHandler(){
		...
		var ajaxConfig = $.extend(true,{},NetstarUI.resultTable.config.default.uploadAllSaveAjax);
		NetStarUtils.ajax(ajaxConfig,function(data){
			if(data.success){
...
NetstarUI.resultTable.getCheckboxAjaxVisible = function(tdData,config){
	...
	if(isNeedGetData){
		...
		NetStarUtils.ajax(ajaxConfig, function(data,ajax){
...
NetstarUI.resultTable.savedHandler = function(config,submitData){
	...
	var saveParams = $.extend(true,{},config.saveAjax.data);
	if(!$.isEmptyObject(config.pageParam)){
		saveParams = NetStarUtils.getFormatParameterJSON(saveParams,config.pageParam);
	}
	console.log(formatSubmitData);
	saveParams.jsonRows = JSON.stringify(formatSubmitData);
...
NetstarUI.resultTable.historyClickHandler = function(ev){
	...
	var ajaxConfig = $.extend(true,{},config.historyAjax);
	NetStarUtils.ajax(ajaxConfig,function(data){
		if(data.success){
			if(debugerMode){
				if(typeof(data)=='undefined'){
...
NetstarUI.resultTable.refreshHistoryPlane = function(historyArray,tdData){
	...
	for(var historyI = 0; historyI<historyArray.length; historyI++){
		...
		// }
		//格式化日期
		if(typeof(trData.whenCreated)=='number'){
			trData.visibleUpdateDate = moment(trData.whenCreated).format(nsVals.default.momentDataTime);
		}else{
			trData.visibleUpdateDate = '';
		}
		//人员处理
		if(typeof(trData.createdByName)=='string'){
			trData.visibleUpdateByName = trData.createdByName;
		}else{
			trData.visibleUpdateByName = '';
		}
		var trHtml = '';
		trHtml += '<td>'+ (historyI+1) +'</td>';
		trHtml += '<td>'+ trData.showValue +'</td>';
...
NetstarUI.resultTable.offlineMode = function(){
	...
	//获取服务端资源数据
	var ajaxConfig = $.extend(true,{},cacheConfig.cachelistAjax);
	NetStarUtils.ajax(ajaxConfig,function(data){
	...
	function getAjaxData(rowData){
		...
		//获取表格数据
		var ajaxConfig = $.extend(true,{},cacheConfig.cacheDataAjax);
		ajaxConfig.data = ajaxData;
		ajaxConfig.context = context;
		NetStarUtils.ajax(ajaxConfig,function(data){
