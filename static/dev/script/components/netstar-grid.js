...
var NetStarGrid = (function () {
			...
			BODY: {
				...
				CONTENTTABLE:
					...
					'<template v-else-if="columnCofig.columnType === \'buildPdf\'">\
						<div class="pt-btn-group">\
							<button class="pt-btn pt-btn-default" @click.self="buildPdfFunc($event, index, columnCofig, row)">{{row[columnCofig.field] ? row[columnCofig.field] : \"生成\"}}</button>\
						</div>\
					</template>'+
					// 标准值输出/多值/多维输出
		...
		getColumnsConfig: function (_gridConfig) {
			...
			for (var columnI = 0; columnI < _columnsConfig.length; columnI++) {
				...
				switch (columnConfig.orderType) {
					case 'asc':
						orderDefaultData = { orderField: columnConfig.field, orderType: 'asc' };
						break;
					case 'desc':
						orderDefaultData = { orderField: columnConfig.field, orderType: 'desc' };
						break;
				}
		...
		getDragColumnsConfig: function (_gridConfig) {
			...
			for(var columnI=0; columnI<columnArray.length; columnI++){
				...
				if(columnData.isResizeWidth){
					...
					columnData.dragStyleObj = {'left': leftWidth + 'px'};
		...
		getRowData: function (_rowData, _gridConfig) {
			...
			$.each(_rowData, function (key, value) {
				...
				if(rowStyleExpress[key]){
					var rowStyleStr = NetStarUtils.getStyleByCompareValue({rowData:_rowData,tdField:key,styleExpress:rowStyleExpress[key]});
				...
				if (typeof (columnConfig) == 'object') {
					...
					if(colStyleExpress[key]){
						var styleStr = NetStarUtils.getStyleByCompareValue({rowData:_rowData,tdField:key,styleExpress:colStyleExpress[key]});
			...
			switch (columnConfig.columnType) {
				...
				case 'timeUnit':
					if(originalValue){
						...
						var returnValue = '';
						if(years){ returnValue += years + '年' }
						if(months){ returnValue += months + '月' }
						if(days){ returnValue += days + '日' }
						if(hours){ returnValue += hours + '时' }
						if(minutes){ returnValue += minutes + '分' }
						if(seconds){ returnValue += seconds + '秒' }
					}
					break;
				case 'func':
	...
	var methodsManager = {
		...
		dragWidth: function (_vueConfig) {
			...
			$handlers.on('mousedown', { vueConfig: _vueConfig }, function (ev) {
				ev.stopPropagation();
				...
				if(!$.isEmptyObject(styleExpress[key])){
					...
					if($.isArray(_vueData.originalRows)){
						if(_vueData.originalRows.length > 0){
							...
							if(!$.isEmptyObject(rowData)){
								styleStr = NetStarUtils.getStyleByCompareValue({rowData:rowData,tdField:key,styleExpress:styleExpress[key]});
							}
        ...
        footer:{
            ...
            togglePageLengthSelect:function(ev, _vueConfig){
				...
				if(setShow){
					...
					$(document).on('click',{pageLengthSelect:_vueConfig.$data.domParams.pageLengthSelect},documentPageLengthSelect);
				}
            },
            //选中 切换页面长度
            ...
            toggleToPageSelect:function(ev, _vueData){
				...
				if(setShow){
					...
					$(document).on('click',{toPageSelect:_vueData.domParams.toPageSelect},documentToPageSelect);
				}
            },
		...
		columnOrderHandler: function (ev, _vueData) {
			...
			if (orderable) {
				...
				if (isServerMode) {
					...
					nsVals.extendJSON(data, { orderField: field, orderType: orderType });
			...
			if(gridId.indexOf('layout')>-1){
				...
				switch(formatHandler.data.templateName){
					case 'businessDataBaseEditor':
						...
						break;
					default:
						NetstarUI.labelpageVm.loadPage(url,formatHandler.data.title, true);
						break;
				}
			}
		},
		...
		buildPdfFunc : function(ev, index, columnCofig, rowData, gridConfig, vueConfig){
			...
			// 判断是生成 还是 预览
			var isBuildPdf = rowData[columnCofig.field] == "生成" ? true : false;
			var editConfig = columnCofig.editConfig;
			if(isBuildPdf){
				...
				var data = data = NetstarComponent.commonFunc.getFormDataByTable(editConfig, 'produceFileAjax');
			...
			// if(!originalRow[columnCofig.field]){
	...
	var tdEditor = {
		...
		getContainer: function (tdData, $td, gridConfig, vueConfig) {
			...
			$container.attr({ ':style': 'containerStyleObj' });
		...
		addRemoveListener: function ($td) {
			...
			$('body').on('mousedown', { this: _this, $td: $td }, this.outClickHandler);
			$('body').on('mousewheel', { this: _this, $td: $td }, this.mouseWheelHandler);
		},
		...
		outClickHandler: function (ev) {
			...
			if(isOut){
				...
				if(ev.target.className == 'pt-dropdown'){
					if(ev.target.parentNode && ev.target.parentNode.className === "pt-input-group pt-select-panel"){
						isOut = false;
					}
				}
			}
