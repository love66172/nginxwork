		...
		load:function(url, callbackFunc){
			...
			loadingTask.promise.then(function(pdf) {
				...
				if(config.id){
					...
					_this.pdfObj[config.id] = {pdf:pdf,config:config};
				}
...
NetstarUI.pdfDialog = (function($){
	...
	function setDefault(_config){
		...
		_config.$selectPage = $('<select id="dialog-pdf-selectpage" class="select-changepage-intitle"   style="float: right; "></select>');
