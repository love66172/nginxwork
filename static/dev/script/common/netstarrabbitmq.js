 ...
 * @Desription: 文件说明
 * @Author: netstar.cy
 * @Date: 2019-08-28 12:02:20
 * @LastEditTime: 2019-09-05 11:11:22
 */
...
var NetStarRabbitMQ = (function(){
    ...
    function init(config){
        ...
        content['x-queue-name'] = NetStarUtils.OAuthCode.get();
    ...
    var sounderSubscribeManage = {
        ...
        subscribe: function(){
            var subscribeConfig = {
                target : '/exchange/cache/platform.cache.mindmap.#',
                unitId : 'NetStarRabbitMQSounder',
                content : {},
                callbackHandler : function(subscribeInfo){
                    // console.warn('思维导图');
                    var sounderConfig = {
                        id : 'mainpage-systeminfo-contianer-sounder',
                        url : getRootPath() + '/public/static/assets/json/song.ogv',
                    }
                    NetstarSounder.play(sounderConfig);
                },
            }
            NetStarRabbitMQ.subscribe(subscribeConfig);
        },
    ...
    function setInfoRemindByCurrent(workitemInfo){
        ...
        var userId = NetstarMainPage.systemInfo.user.userId;
    ...
    function inPageHandler(pageParams, currentPage){
        ...
        switch(pageType){
            ...
            case 'workflow':
                ...
                if(pageConfig.isNew){
                    NetstarUI.flowChartViewer.tab.refreshWorkflow(pageConfig.name);
    ...
    function setTemplateSubscribe(subConfig){
        console.warn('setTemplateSubscribe');
        // 订阅参数
