 ...
 * 	mxGraph
 ***/
NetstarUI.flowChartViewer = (function(){
	...
	function getViewerCellsCodecBySource(config, styleSheetXMLDoc){
		...
		// 初始化图像
		if(typeof(NetstarUI.flowChartViewer.graphs[config.containerId])=="object"){
		...
		NetstarUI.flowChartViewer.graphs[config.containerId] = graph;
	...
	function initGraph(config, styleSheetXMLDoc){
		...
		if(config.readonly == false){
			...
			config.$workflowContainer.parent().children('.pt-btn-group').children().eq(0).on('click',function(ev){
				NetstarUI.flowChartViewer.graphs[config.containerId].zoomIn();
			});
			...
			config.$workflowContainer.parent().children('.pt-btn-group').children().eq(1).on('click',function(ev){
				NetstarUI.flowChartViewer.graphs[config.containerId].zoomOut();
			});
	...
	function showGroupByCells(graphConfig){
        ...
        // }
        if(typeof(NetStarRabbitMQ) == "object" && typeof(NetStarRabbitMQ.subscribeQueue) == "object" && typeof(NetStarRabbitMQ.subscribeQueue[graphConfig.unitId])!="object"){
	        var subscribeConfig = {
	...
	function showWorkitemsListGraph(listConfig){
		...
		model = new mxGraphModel(); //空的图像实体
		graph = new mxGraph(container, model);  //图像管理器
		NetstarUI.flowChartViewer.graphs[containerId] = graph;
		activityInfo.activityType = Number(activityInfo.activityType);
		// 获得图形
		var listConfig = {
			cell : cell,
	...
	var tab = {
		...
		refreshGroupByWorkitemId: function(obj){
			...
			NetstarUI.flowChartViewer.init(viewerConfig);
		},
		...
		refreshGroupByWorkflowId: function(obj){
			...
			NetstarUI.flowChartViewer.initMultilevel(viewerConfig, obj);
		},
		// 初始化添加按钮方法
		...
		getWorkflowContainer: function(viewerConfig){
			...
			if($('body').hasClass('pt-veritical')){
				headHeight = $('.pt-header .pt-tabbar').outerHeight();
			}
