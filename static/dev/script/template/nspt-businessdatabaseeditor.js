 ...
 * @Desription: 文件说明
 * @Author: netstar.cy
 * @Date: 2019-08-28 14:22:22
 * @LastEditTime: 2019-08-30 17:30:04
 */
NetstarTemplate.templates.businessDataBaseEditor = (function ($) {
   var dialogForAdd = function (outerConfig) {
      var config = {};
      var $tabPanelArr = [];
      var serverData = {};
      var isError = false;
      ...
      var imgUpload = {}; // 上传图片 lyw 20190916 此方法从自执行方法到现在方法 之前imgUpload直接设置的变量不可以了 所以：设置config后设置此变量
      //初始化
      ...
      var imgUpload = {
         ...
         uploadFiles: function (files) {
            ...
            //this.uploadConfig.ajax.src = this.uploadConfig.ajax.src + '?visibilityLevel=2';
      ...
      function formatForm(formConfig, formName) {
         ...
         $.each(formConfig.field, function (index, item) {
            ...
            //设置自动聚焦
            if (!isSetFocus && item.type != 'hidden' && !item.hidden && $.inArray(item.type, shownFields) != -1) {
               isSetFocus = true;
                     ...
                     case 'uploadCover':
                        ...
                        if($.isArray(serverData[config.uploadCoverConfig.keyField])){
                           ...
                           for(var imgI=0; imgI<imgArray.length; imgI++){
                              ...
                              //cy 20190830 图片后面加token只用于跨域情况下使用 start ------
         ...
         var vm = new Vue({
            ...
            methods: {
               close: function (isNotVerify) {
                  //右上角关闭按钮方法逻辑
                  /**
                   * 1.如果当前弹出框的值未进行修改操作直接关闭当前弹出框
                   * 2.如果当前弹出框的值进行了修改操作需要弹出操作提示框 
                   */
                  var _this = this;
               ...
               saveAndAdd: function (data) {
                  var $btn =  $(data.event.currentTarget);
                  var _this = this;
                  var config = this.config;
               ...
               if(!config.readonly){
                  btnArray.push({
                     ...
                     handler:function(data){
                        $(data.event.currentTarget).attr('disabled',true);// 按钮设置只读
                        _this.save($(data.event.currentTarget));
                     },
      ...
      function setDefaultData(cb) {
         ...
         if (typeof config.pageParam != 'undefined') {
            ...
            /**************** sjj 20190926 通过url解析出来的传参editModel start*/
            if(config.pageParam.editModel){
               switch(config.pageParam.editModel){
                  case '2':
               ...
               delete config.pageParam.editModel;
            }
             /**************** sjj 20190926 通过url解析出来的传参editModel end*/
            //只读
         ...
         if (typeof config.getValueAjax != 'undefined') {
            //SJJ 20190909 如果参数来源是copyParams
            var sourcePageParams = $.extend(true,{},config.pageParam);
            if(!$.isEmptyObject(sourcePageParams)){
               if(!$.isEmptyObject(sourcePageParams.copyParams)){
                  sourcePageParams = $.extend(true,{},sourcePageParams.copyParams);
               }
            }
				// copyParams在root下边的参数找不到所以把copyParams放在外层 lyw 20190925
            var ajax = nsVals.getAjaxConfig(config.getValueAjax, sourcePageParams, {
            ...
            nsVals.ajax(ajax, function (res, _ajaxConfig) {
               if (res.success) {
                  ...
                  if(editorType=='copyAdd'){
                     // lyw 20190925 如果当前操作是复制新增
                     config.defaultData.copyParams = res[ajax.dataSrc];
                     // 删除主表的主键 idField
                     if(config.idFieldNames && config.idFieldNames.root){
                        delete config.defaultData.copyParams[config.idFieldNames.root];
                     }
                  }else{
                     config.defaultData = res[ajax.dataSrc];
                  }
                  ServerDataByAjax = $.extend(true,{},res[ajax.dataSrc]);  //sjj 20190723 服务端返回数据值
                  //如果是复制新增
