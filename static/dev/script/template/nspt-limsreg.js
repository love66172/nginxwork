...
NetstarTemplate.templates.limsReg = (function ($) {
   ...
   //给detail中的左侧列表添加数据
   //处理tab数据 增删改方法
   function tabComponentFunc(_config,operatorType,rowData,_rowIndex){
      ...
      switch(operatorType){
         ...
         case 'copyAdd':
            ...
            fillValuesTabByRowData(data,{package:_config.package});
            //var data = getTabData(voIdArr,listIdArr,tabComponentData);
            /*if(data){
               data['NETSTAR-TIMER'] = moment().format('x');
               NetStarGrid.addRow(data,_config.detailLeftComponent.id);
   ...
   function fillValuesByVo(_config,valueData){
      ...
      for(var voId in componentData){
         ...
         NetstarTemplate.commonFunc.vo.clearValues(voId,false);//先清空再赋值
         if(isRoot){
            fillValuesByVoId(voId,valueData);
         }else{
            if($.isArray(valueData[voConfig.parent])){
               if(valueData[voConfig.parent].length > 0){
                  fillValuesByVoId(voId,valueData[voConfig.parent][0]);
               }
            }
         }
      }
   ...
   function commonChangeHandlerByTabVo(data){
      ...
      var formData = NetstarComponent.getValues(formConfig.formID);
         ...
         if(isRoot){
            ...
            if($.isArray(value[detailLeftComponent.keyField])){
               ...
               for(var rowI=gridDataArr.length; rowI--; rowI>=0){
                  ...
                  NetStarGrid.addRow(gridDataArr[rowI],detailLeftComponent.id);
               }
               //给右侧tab中的grid赋值
            ...
            if(keyField == tempalteConfig.detailLeftComponent.keyField){
               ...
               for(var rowI=valueData.length; rowI--; rowI>=0){
                  ...
                  NetStarGrid.addRow(valueData[rowI],detailLeftComponent.id);
               }
               //给右侧tab中的grid赋值
               var components = tempalteConfig.tabConfig.components;
               ...
               for(var componentId in components){
                  switch(components[componentId].type){
                     ...
                     case 'vo':
                        NetstarTemplate.commonFunc.vo.clearValues(components[componentId].id,false);//先清空再赋值
                        fillValuesByVoId(components[componentId].id,valueData);
                        break;
                  }
               }
            }  
