 ...
 * @LastEditTime: 2019-10-26 17:51:49
 */
/*
	nsMindjetToJS
*/
		...
		switch(_type){
			...
			case 'upload':
				...
				_fieldConfig.previewAjax = {
					url : projectObj.system.prefix.url + '/files/pdf/',
					type : 'GET',
				}
...
//--------------------------------------------------wxk公共弹窗------------------------------------------------------------		
    ...
    if(typeof(controllerObj.defaultMode)=='string'){
    	...
    	//defaultMode类型
    	var dataUserModeKey = [
    		'dialog','valueDialog','confirm','toPage','loadPage','changePage','ajaxDialog','component','print','custom','templatePrint','workflowViewer','workflowSubmit','newtab','viewerDialog','successMessage','dataImport','excelImportVer2','multiDialog','ajaxAndSend','business','previewFile'
    	];
	...
	function previewFileInit(callback,obj){
		...
		NetStarUtils.ajax(ajaxConfig, function(res, _ajaxConfig){
			...
			for(var i=0; i<fileFieldsArr.length; i++){
				if(typeof(data[fileFieldsArr[i]]) != "undefined" && data[fileFieldsArr[i]]){
					ids.push(data[fileFieldsArr[i]]);
				}
			}
			if(ids.length == 0){
