 ...
 * @Desription: 文件说明
 * @Author: netstar.cy
 * @Date: 2019-08-22 20:59:19
 * @LastEditTime: 2019-08-22 21:39:27
 */
    ...
    var table = {
        ...
        getTableHtml : function(config){
            ...
            for(var i=0; i<data.length; i++){
                ...
                for(var key in data[i]){
                            ...
                            case 'select':
                                ...
                                for(var h=0; h<subdata.length; h++){
                                    if(subdata[h][editConnfig.valueField] === data[i][key]){
                                        editHtml = subdata[h][editConnfig.textField];
                                        break;
                                    }
                                }
                                break;
                    ...
                    var tdStr = '<td style="width:' + width + ';" ns-row="' + i + '" ns-col="' + col + '" ns-id="' + editConnfig.field + '">' + editHtml + '</td>';
                    tdRowHtml += tdStr;
                }
                rowsHtml += tdRowHtml;
            }
        ...
        getPanelHtml : function(config){
            var html = '<div class="pt-standard-header" id="' + config.headerId + '"></div>'
                        + '<div class="pt-standard-body" id="' + config.bodyId + '" style="height:300px;overflow-y:auto;">'
        ...
        getSaveData : function(id){
            ...
            if(value){                        
                // saveData = value;
                var saveData = $.extend(true, {}, stdInputDataManager.data);
                saveData.valueList = value.obj;
            }
            return saveData;
        },
        // 获取jQ表格
        ...
        getDialogSize : function(config){
            ...
            var $window = $(window);
            if(rowsHeight > $window.height()-100){
                rowsHeight = $window.height()-100;
            }
            return {
                width : colsWidth,
                height : rowsHeight,
    ...
    var stdInputDataManager = {
        ...
        getSubdataByAjax:function(callbackFunc){
            ...
            for(var i = 0; i < attrList.length; i++){
                var isNumerical = typeof(attrList[i].isNumerical) == 'string'?attrList[i].isNumerical:attrList[i].isNmerical;
                if(isNumerical == '0'){
                    //如果是数字则跳过
                    continue;
                }
        ...
        getTableConfig:function(){
            var _this = this;
            var readonly = false;
            if(stdInputDataManager.config.readonly){
                readonly = true;
            }
            //获取列配置
            var _config = {};
            ...
            for(var i = 0; i < _this.data.attrList.length; i++){
                ...
                var isNumerical = typeof(attr.isNumerical) == 'string'?attr.isNumerical:attr.isNmerical;
                var column = {
                    field :     attr.cal,
                    title :     attr.name,
                    type:       isNumerical == '1'?'select':'text',
                    readonly :  readonly,
                };

                if(column.type == 'select'){
                    var subData = _this.subDataObj[attr.cal].rows.length > 0 ?_this.subDataObj[attr.cal].rows[0].children: [];  //字段可能有错误
                    column.subData      =  subData;
