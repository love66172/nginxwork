 ...
 * @Desription: 文件说明
 * @Author: netstar.cy
 * @Date: 2019-09-28 11:10:03
 * @LastEditTime: 2019-09-28 16:29:30
 */
/****
***下拉树  标题 表格 分类信息
****
...
NetstarTemplate.templates.businessDataBase = (function(){
	...
	function dialogBeforeHandler(data,templateId){
		...
		if(typeof(data.controllerObj)=='object'){
			...
			if(typeof(config.pageParam)=='object' && typeof(data.controllerObj.func)=='object'){
			...
			switch(config.paramMode){
				case 'list':
					...
					if(currentQueryParams.queryParams.filtermode == 'quickSearch'){
						...
						var queryId = 'query-'+tableId;
						var formJson = NetstarComponent.getValues(queryId,false);
						paramJson[currentQueryParams.queryParams.filtermode] = formJson[currentQueryParams.queryParams.filtermode];
					}
					break;
				default:
	...
	function setContainerHeight(config){
		...
		if(config.title){
			//biaotigaodu
			pageHeight = pageHeight + 28;
		}
		commonPanelHeight = $(window).outerHeight()-pageHeight-20;
	}
		...
		showConfig.closeHandler = (function(){
			return function(){
				...
				if(configObj.displayMode == 'tree'){
					NetstarTreeList.ztreeManager.ajaxByTree(NetstarTreeList.configs[configObj.gridId].treeConfig);
				}else{
	...
	function componentInit(_config,_componentConfig){
		...
		*/
		...
		for(var componentI=0;componentI<config.components.length; componentI++){
			...
			switch(componentData.type){
				case 'list':
					listAjaxConfig = componentData.ajax;
		...
		switch(componentConfig.componentClass){
			case 'list':
				...
				if(displayMode == 'tree'){
					...
					var queryConfig = NetStarUtils.getListQueryData(componentsConfig.list[0].columns,{id:componentConfig.container,value:defaultValue});
					//把业务组件的配置参数作为入参
					componentConfig.ajax = listAjaxConfig;
					componentConfig.pageParam = _config.pageParam;
					componentConfig.queryConfig = queryConfig;
					NetstarTreeList.init(componentConfig);
				}else{
		...
		var callbackFunc = {
			selectHandler:function(){
				var selectData = [];
				if(displayMode == 'tree'){
					var ztreeObj = NetstarTreeList.configs[componentConfig.container].treeConfig.ztreeObj;
					switch(componentConfig.selectMode){
						case 'single':
							selectData = ztreeObj.getSelectedNodes();
							break;
						case 'checkbox':
						case 'multi':
							selectData = ztreeObj.getCheckedNodes();
							break;
					}
				}else{
					var tableId = componentConfig.container;
					selectData = getSelectedData(tableId);
				}
				return selectData;
			},
			selectedComplateHandler:(function(){
				return function(_ajaxData){
					...
					var distinctField = componentConfig.editorConfig.distinctField;
		...
		if(componentBtnConfig[config.id].add){
			callbackFunc.addHandler = function(_func){
				...
				$(document).off('keyup',documentKeyupHandler);
				var tableId = componentConfig.container;
				getAjaxToPage({gridId:tableId,templateId:config.id,displayMode:displayMode,data:{gridId:tableId,templateId:config.id},type:'add'});
				//console.log('add');
			}
		}
		if(componentBtnConfig[config.id].edit){
			callbackFunc.queryHandler = function(){
				...
				var selectedData = [];
				if(displayMode == 'tree'){
					var ztreeObj = NetstarTreeList.configs[tableId].treeConfig.ztreeObj;
					switch(componentConfig.selectMode){
						case 'single':
					...
					selectedData = getSelectedData(tableId);
				}
				if(selectedData.length > 0){
					componentBtnConfig[config.id].query = componentBtnConfig[config.id].edit;
					componentBtnConfig[config.id].query.editorType = 'query';
					getAjaxToPage({gridId:tableId,templateId:config.id,displayMode:displayMode,data:{gridId:tableId,templateId:config.id,rowData:selectedData},type:'query',templateId:config.id});	
				}else{
