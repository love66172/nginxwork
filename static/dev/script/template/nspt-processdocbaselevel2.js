...
NetstarTemplate.templates.processDocBaseLevel2 = (function ($) {
   ...
   function initContainer(_config){
      ...
      if($('#netstar-main-page').length > 0){
         ...
         _config.gridHeight = $(window).outerHeight()  - 350;
      }
               ...
               if(componentData.type != 'tab' && componentData.type != 'btns'){
                  ...
                  if(componentData.parent){
                     ...
                     if(componentData.parent != 'root'){
                        ...
                        componentData.params.selectedHandler = NetstarTemplate.templates.processDocBaseLevel2.gridSelectedHandler;
                        componentData.listDrawHandler = NetstarTemplate.templates.processDocBaseLevel2.listDrawHandler;
      ...
      if(!$.isEmptyObject(_config.getValueAjax)){
         ...
         NetStarUtils.ajax(getValueAjaxConfig,function(res,ajaxPlusData){
            if(res.success){
               ...
               NetstarTemplate.templates.processDocBaseLevel2.data[ajaxPlusData.plusData.templateId].config.serverData = serverData;
   ...
   return {
      ...
      listDrawHandler:function(vueData){
         ...
         if(existIndex >-1){
            if(vueData.$options.id == detailLeftComponent.id){
               //刷新右侧数据
               if($.isArray(leftGridData[existIndex][keyField])){
                  NetStarGrid.refreshDataById(templateConfig.detailRigthComponent.id,leftGridData[existIndex][keyField]);
               }
            }else if(vueData.$options.id == templateConfig.detailRigthComponent.id){
