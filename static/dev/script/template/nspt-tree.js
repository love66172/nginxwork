NetstarTemplate.tree = (function(){
    ...
    var eventManage = {
        ...
        // 计算滚动条位置 使当前选中节点显示在面板内
        setTreeScrollTop : function($node, $tree){
            var position = $node.position();
            $tree.scrollTop(position.top); // 移动到顶部 一般不会显示在顶部 原因滚动条没有那么长
        },
        // 功能面板查询
        search : function(_config){
            ...
            if(!$.isEmptyObject(searchNode)){
                ...
                $node.trigger('click');
                eventManage.setTreeScrollTop($node, _config.$tree)
            }else{
    ...
    var treeManage = {
        ...
        getFormatData : function(){
            ...
            // 添加未分类
            if(config.unfieldId.length > 0 && zNodes[0]){
                var unfiledNode = {
                    isPrev : true,
                    isNext : false,
                    isInner : false,
        ...
        initTree : function(){
            ...
            if(config.zNodes.length == 0){
                nsAlert('没有树节点数据');
                console.error('没有树节点数据');
                return;
            }
            config.saveData = [];
            config.addNodeNum = 0;
    ...
    var configManage = {
        ...
        setDefault : function(){
            ...
            if(config.unfieldId.length > 0){
                config.isUnfiled = true;
            }else{
                config.isUnfiled = false;
            }
        },
        // 设置config 通过是否只读状态
