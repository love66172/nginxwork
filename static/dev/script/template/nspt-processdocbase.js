 ...
 * @LastEditTime: 2019-09-07 14:11:07
 * @Desription: 文件说明
 */
NetstarTemplate.templates.processDocBase = (function ($) {
   ...
   var busComponents = {
      ...
      addInfoDialog: {
         ...
         setConfig: function (outerConfig, extraData, btnEnglishName) {
            ...
            if(typeof(pageConfig)=='undefined'){pageConfig = config;}
            ...
            currentConfig.closeHandler = function (data) {
               ...
               NetstarTemplate.hiddenDialogHandler(currentConfig);//sjj 20190814 关闭弹框调用关闭快捷键
            };
   ...
   function getBtnsConfig(btns) {
            ...
            return function (data) {
              ...
              //data.controllerObj.parameterFormat
               ...
               if(data.controllerObj){
                  ...
                  if(data.controllerObj.requestSource == 'none'){
                     isValidatParams = false;
                  }
               }
            ...
            return function (data, plusData) {
               ...
               switch (data.objectState) {
                  case NSSAVEDATAFLAG.DELETE:
                     ...
                     currentConfig.formatComponentsArr = [];
                     formatComponents(currentConfig.components, currentConfig);
                     renderComponents();
                     break;
      ...
      for (var i = 0; i < btns.field.length; i++) {
         ...
         if (typeof item.functionConfig == 'object' && $.inArray(item.functionConfig.defaultMode, defaultModeArr) != -1) {
            ...
            btnsArray[i].handler = function (data) {
               var packageName = $(data.event.currentTarget).closest('.btn-group').attr('ns-tempalte-package');
               var addInfoDialogConfig = NetstarTemplate.templates.configs[packageName];
               var currentFunctionConfig = data.data.functionConfig;
               var presuffix = currentFunctionConfig.suffix;
               var currentDefaultMode = currentFunctionConfig.defaultMode;
               getOtherPageConfig(presuffix, currentDefaultMode, {keyField: currentFunctionConfig.keyField,package: addInfoDialogConfig.package.replace(/-/g, '.')}, currentFunctionConfig);
            };
   ...
   function buildContainer(_config, _listDrag) {
      ...
      $.each(config.components, function (index, item) {
         ...
         switch (item.type) {
            ...
            case 'btns':
               ...
               $insertion.append('<div class="btn-group" id="' + item.id + '" ns-tempalte-package="'+config.package+'"></div>');
               $currentPosition.append($currentContainer);
               break;
            case 'tab':
               break;
      ...
      if(config.isListTabs){
         ...
         //style="height:'+config.remainHeight+'px;max-height:'+config.remainHeight+'px;"
      ...
      for (var index = 0, len = config.formatComponentsArr.length; index < len; index++) {
         ...
         switch (item.renderType) {
            ...
            case 'grid':
               ...
               gridConfig.ui.height = getGridHeight(config);
               if(config.isListTabs == true){
                  gridConfig.ui.height = config.pageHeight - $('#'+config.containerHeaderId).outerHeight() - 120;
               }
               //发送函数名用来获取数据
   ...
   function addBtnsInfo(templateOptions) {
      for (var index = 0; index < templateOptions.field.length; index++) {
         ...
         if (item.functionConfig.defaultMode == 'addInfoDialog') {
            ...
            var suffix = functionConfig.suffix;
            var defaultMode = functionConfig.defaultMode;
            config.idFieldNames['root.'+functionConfig.keyField] = 'id';
   ...
   function setDefaultData(cb) {
      ...
      /*****sjj 20190926 根据界面来源的editModel 判断当前界面的逻辑 start */
      config.editModel = 0;//默认为0 
      if(!$.isEmptyObject(config.pageParam)){
         if(config.pageParam.editModel){
            config.editModel = config.pageParam.editModel;
            switch(config.editModel){
               case '2':
      ...
      /************ sjj 20190926 根据界面来源的editModel 判断当前界面的逻辑 end*/
      ...
      if (typeof config.getValueAjax != 'undefined' && !$.isEmptyObject(config.getValueAjax)) {
         ...
         if(!$.isEmptyObject(config.getValueAjax.data)){
            //sjj 20190527
            config.getValueAjax.data = NetStarUtils.getFormatParameterJSON(config.getValueAjax.data,config.pageParam);
         }else{
         ...
         NetStarUtils.ajax(config.getValueAjax, function (res) {
            if (res.success) {
               ...
               if(NetstarTemplate.templates[templateName].aggregate){
                  if(NetstarTemplate.templates[templateName].aggregate[config.package]){
                     NetstarTemplate.templates[templateName].aggregate[config.package].config = config;
                  }
               }
            } else {
               nsalert('服务器端返回数据错误');
   ...
   function getPageData(package, _formVerify, _setObjectState) {
         ...
         switch (item.type) {
            case 'vo':
               ...
               */
               ...
               break;
            case 'list':
               fillValue = _formVerify ? verifyGridData(NetStarGrid.dataManager.getData(item.id), config, item) : NetStarGrid.dataManager.getData(item.id);
               /**********************sjj 20190521 start */
      ...
      if(_setObjectState === false){}else{
         pageData = nsServerTools.getObjectStateData(config.serverData, pageData, config.idFieldNames);
         nsServerTools.setObjectStateByFormatData(pageData, config.components); // lyw 20190903 业务组件数据发生改变修改objectState
      }
   ...
   //检验表格数据是否合法
   function verifyGridData(gridData, config, gridConfig) {
      ...
      var mainGridConfigManager = NetStarGrid.configs[gridConfig.id];
   ...
   //获取页面剩余高度
   function getGridHeight(_config) {
      //cy 20190907修改高度计算方式 start -----
      var gridHeight = 0;
      var containerOffset = config.$appendContainer.offset();
      var availableHeight = $(window).innerHeight() - containerOffset.top;       //容器内部可用高度 
      //var availableWidth = $(window).innerWidth() - containerOffset.left;      //       可用宽度      
      ...
      for(var i = 0; i < config.components.length; i++ ){
         var componentConfig = config.components[i];
         switch(componentConfig.type){
            case 'vo':
               if(componentConfig.position == "header-right"){
                  componentLength.inlineForm ++;
                  usedHeight.form += 66;  //向左对齐的表单组件时单行的高度
               }else{
                  
               }
               break;
            case 'list':
               componentLength.grid ++;
               //主要用grid填充剩下的区域，所以表格不占用已有位置
               break;
            case 'btns':
               //有按钮则减去按钮占用高度，假设为60 目前假设只有一个
               componentLength.btns ++;
               usedHeight.btns = 60;
               break;
            default:
               break;
      ...
      if(componentLength.grid > 1 &&  componentLength.form == 0){
         //如果表格多于一个 且没有高度可变的表单 则直接分配可用高度 需要减去按钮组高度和单行表单的高度
         var allGridHeight = availableHeight - usedHeight.btns - usedHeight.inlineForm;
         gridHeight = allGridHeight / componentLength.grid;
         //gridHeight = gridHeight - 10;
      }else{
         gridHeight = availableHeight - componentLength.btns - 160;  //160是个临时的数据，用于替代掉表单的可能高度
      }
      config.remainHeight  = gridHeight;
      return gridHeight;

      //cy 20190907修改高度计算方式 end -----
