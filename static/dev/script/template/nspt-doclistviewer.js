 ...
 * @Desription: 文件说明
 * @Author: netstar.cy
 * @Date: 2019-10-16 17:54:15
 * @LastEditTime: 2019-10-20 22:20:30
 */
/*
	*单据详情模板
	*sjj 20190214
...
NetstarTemplate.templates.docListViewer = (function(){
	...
	function dialogBeforeHandler(data,templateId){
		...
		if(controllerObj.requestSource == 'none'){
			data.value = {};
		}else{	
			...
			if(selectedData.selectedList.length > 0){
				if(!$.isEmptyObject(data.controllerObj)){
					if(data.controllerObj.requestSource != 'checkbox'){
						...
						//读取子表选中值
						/*if(config.detailListManager){
							for(var detailId in config.detailListManager.keyField){
								var detailValue = NetStarGrid.getSelectedData(detailId);
								if($.isArray(detailValue)){
									voData[config.detailListManager.keyField[detailId]] = detailValue;
								}
							}
						}*/
			...
			if(controllerObj.requestSource == 'thisVo'){
				if(config.detailListManager){
					for(var detailId in config.detailListManager.keyField){
						var detailValue = NetStarGrid.getSelectedData(detailId);
						if($.isArray(detailValue)){
							data.value[config.detailListManager.keyField[detailId]] = detailValue;
						}
					}
				}
			}
			if(operatorObject !='root'){
				...
				var currentGridId = '';
				for(var fieldI in keyFieldJson){
					if(keyFieldJson[fieldI] == operatorObject){
						currentGridId = fieldI;
					}
				}
				//获取选中值
				var gridSelectedData;
				if(config.detailListManager.listConfig.id == currentGridId){
					gridSelectedData = NetStarGrid.getSelectedData(currentGridId);
				}else{
					NetstarBlockList.getSelectedData(currentGridId);
				}
				if(!$.isArray(gridSelectedData)){gridSelectedData = [];}
				if(controllerObj.requestSource == 'checkbox'){
					data.value = {selectedList:gridSelectedData};
				}else{
					if(gridSelectedData.length == 1){
						data.value = gridSelectedData[0];
					}
				}
			}
		}
			...
			if($.isArray(res)){
				...
				switch(res.objectState){
					case NSSAVEDATAFLAG.DELETE:
						...
						if(config.mode == 'expandList'){
							var listConfig = config.detailListManager.listConfig;
							var blockListConfig = config.detailListManager.blockListConfig;
							NetStarGrid.refreshDataById(listConfig.id,[]);
							NetstarBlockList.refreshDataById(blockListConfig.id,[]);
						}else{
					...
					case NSSAVEDATAFLAG.ADD:
						...
						if(config.mode == 'expandList'){
							var listConfig = detailListManager.listConfig;
							var blockListConfig = detailListManager.blockListConfig;
							if($.isArray(res[listConfig.keyField])){
								NetStarGrid.refreshDataById(listConfig.id,res[listConfig.keyField]);
							}else{
								NetStarGrid.refreshDataById(listConfig.id,[]);
							}
							if($.isArray(res[blockListConfig.keyField])){
								NetstarBlockList.refreshDataById(blockListConfig.id,res[blockListConfig.keyField]);
							}else{
								NetstarBlockList.refreshDataById(blockListConfig.id,[]);
							}
						}else{
			...
			switch(listArray[listI].type){
				...
				default:
					...
					if(!$.isEmptyObject(listData.params)){
						...
						if(listData.params.isCheckSelect){
							customUi.isCheckSelect = listData.params.isCheckSelect;
						}
					}
			...
			if(formJson.filtermode == 'quickSearch'){
				...
				if(!$.isEmptyObject(queryConfig)){
					if(formJson[formJson.filtermode]){
						if(queryConfig.type == 'business' && queryConfig.isReadOutputFields !== true){
	...
	*/
	...
	//获取详情表数据
	function getDetailsData(data,_gridConfig,isTimeout){
		// data
		if(typeof(data)=="undefined"||$.isEmptyObject(data)){
			// 主表没有选中数据 lyw 20190226
			return ;
		}
		isTimeout = typeof(isTimeout)=='boolean' ? isTimeout : false;//默认不判断是否需要添加延时 
		//isTimeout 只有在当前滚动的时候会添加延时加载，原因滚动太快频繁的时候会频繁的发送ajax请求
		...
		function sendAjaxConfig(ajaxOptions){
			NetStarUtils.ajax(ajaxOptions,function(res){
				...
				if(res.success){
					...
					if(config.mode == 'expandlist'){
						var listConfig = detailListManager.listConfig;
						var blockListConfig = detailListManager.blockListConfig;
						if($.isArray(resData[listConfig.keyField])){
							NetStarGrid.refreshDataById(listConfig.id,resData[listConfig.keyField]);
						}else{
							NetStarGrid.refreshDataById(listConfig.id,[]);
						}
						if($.isArray(resData[blockListConfig.keyField])){
							NetstarBlockList.refreshDataById(blockListConfig.id,resData[blockListConfig.keyField]);
						}else{
							NetstarBlockList.refreshDataById(blockListConfig.id,[]);
						}
					}else{
		...
		if(!$.isEmptyObject(detailListManager.tab)){
			...
			sendAjaxConfig(ajaxOptions);
		}else{
			//如果没有tab 则是两个或多个并列显示的子表
			var _ajaxOption = {};
			var _ajaxOptionLength = 0;
			//在子表中循环查找
			for(var i=0; i<detailListManager.config.length; i++){
				if(detailListManager.config[i].ajax){
					_ajaxOption = detailListManager.config[i].ajax;
					_ajaxOptionLength++;
				}
			}
			if(_ajaxOptionLength != 1){
				console.error('有多个子表存在ajax', detailListManager.config );
			}
			var ajaxOptions = getAjaxConfig(_ajaxOption,data,{source:'component',templateId:templateId})
			sendAjaxConfig(ajaxOptions);
		}
	}
	//行选中
	...
	function drawHandlerByMainGrid(_vueData){
		...
		config = NetstarTemplate.templates.configs[grid.gridConfig.package];
	...
	function componentsPanelInit(){
		if(config.mode == 'expandlist'){
			var commonPanelHeight = config.templateCommonHeight-20;

			var mainComponent = config.mainComponent;
			var detailListConfig = config.detailListManager.listConfig;
			var detailBlockListConfig = config.detailListManager.blockListConfig;
			
			mainComponent.isAjax = true;
			mainComponent.params = {
				height:parseFloat(commonPanelHeight/2),
				drawHandler:drawHandlerByMainGrid,
				isCheckSelect:true,
			};

			detailListConfig.params = {
				height:parseFloat((commonPanelHeight - 50)/2),
				isCheckSelect:true,
			};

			detailBlockListConfig.params = {
				height:commonPanelHeight
			};

			var mainGridConfig = {};
			mainGridConfig[mainComponent.id] = mainComponent;
			var secondGridConfig = {};
			secondGridConfig[detailListConfig.id] = detailListConfig;
			var secondBlockListConfig = {};
			secondBlockListConfig[detailBlockListConfig.id] = detailBlockListConfig;

			NetstarTemplate.commonFunc.list.initList(mainGridConfig,config);
			NetstarTemplate.commonFunc.list.initList(secondGridConfig,config);
			NetstarTemplate.commonFunc.blockList.initBlockList(secondBlockListConfig,config);

			var btnJson = config.btnKeyFieldJson['root'];
			var btnConfig = {};
			btnConfig[btnJson.id] = btnJson;
			NetstarTemplate.commonFunc.btns.initBtns(btnConfig,config);
		}else{
			//判断是否存在isMainDbAction 绑定在双击事件的按钮
					...
					drawHandler:function(_vueData){
						...
						if($.isArray(rowsData)){
							...
							if(selectedIndex > -1){
								...
								//查找按钮是否设置了禁用
								NetStarUtils.setBtnsDisabled(btnsArray,data);
								getDetailsData(originalRows[selectedIndex+startI],config.mainDataConfig,true);
							}else{
								var keyFieldJson = config.detailListManager.keyField;
								for(var listId in keyFieldJson){
									NetStarGrid.resetData([], listId);
	...
	function initContainer(){
		...
		var pageHtml = '';
		if(config.mode == 'expandlist'){
			for(var d=0; d<config.detailListManager.config.length; d++){
				var componentData = config.detailListManager.config[d];
				switch(componentData.type){
					case 'list':
						config.detailListManager.listConfig = componentData;
						break;
					case 'blockList':
						config.detailListManager.blockListConfig = componentData;
						break;
				}
			}
			var btnHtml = '';
			...
			pageHtml = '<div class="pt-container">'
							+'<div class="pt-main-row">'
								+'<div class="pt-main-col" ns-position="left">'
									+'<div class="pt-panel">'
										+'<div class="pt-panel-container" id="'+config.mainComponent.id+'" ns-level="1"></div>'
										+'<div class="pt-panel-container" id="'+config.detailListManager.listConfig.id+'" ns-level="2"></div>'
									+'</div>'
								+'</div>'
								+'<div class="pt-main-col pt-col-auto" ns-position="right">'
									+'<div class="pt-panel">'
										+'<div class="pt-panel-container" id="'+config.detailListManager.blockListConfig.id+'" ns-level="2"></div>'
									+'</div>'
								+'</div>'
							+'</div>'
							+btnHtml
			...
			pageHtml = titleHtml + voHtml + contentHtml + btnsHtml;
		}
		...
		if(config.$container){
			config.$container.html('<div class="pt-main doclistviewer '+templateClassStr+' '+modeClassStr+'" id="'+config.id+'" ns-package="'+config.package+'">'+pageHtml+'</div>');//输出面板
		}else{
			$container.prepend('<div class="pt-main doclistviewer '+templateClassStr+' '+modeClassStr+'" id="'+config.id+'" ns-package="'+config.package+'">'+pageHtml+'</div>');//输出面板
		}
	...
	function setDefault(config){
		...
		for(var componentI=0; componentI<config.components.length; componentI++){
			...
			var containerId = componentData.id;//定义容器id
			if(typeof(containerId)=='undefined'){
				containerId = config.id+'-'+componentData.type+'-'+componentI;
			...
			if(componentData.parent == 'root' || componentData.keyField == 'root'){
				...
				config.mainComponent.quickquery = {config:gridQueryPanelConfig}
			}
			switch(componentData.type){
				case 'tab':
			...
			componentsConfig[componentData.type][componentData.id] = componentData;
		}
	}
	function clearVals(){
		componentsConfig = {
	...
	function init(_config){
		...
		commonHeight = _config.templateCommonHeight;//屏幕容器的高-标签高-标题高-底部按钮高
		...
		if(detailListManager.config.length == 0){
			...
			//主表带查询条件减去查询条件的高度 减去tab标题的高度
			var tabHeight = commonHeight-48-34-10;
			var listHeight = parseFloat(tabHeight/2);
			mainListHeight = listHeight+34;
			detialListHeight = listHeight;
		}
		//if(config.mode == 'horizontal'){
			//mainListHeight = commonHeight;
	...
	return {
		...
		gridSelectedHandler:function(data, $data, vueData, gridConfig, componentData){
			if(componentData && componentData.keyField == "root"){
				getDetailsData(data, gridConfig);
			}
		}
	}
