 ...
 * @Desription: 文件说明
 * @Author: netstar.cy
 * @Date: 2019-08-22 16:16:40
 * @LastEditTime: 2019-08-28 17:17:17
 */
var nsVals = {};
...
nsVals.getAjaxConfig = function (ajaxConfig, ajaxData, options) {
	...
	if (typeof (options) == 'object') {
		...
		if (options.pageParam) {
			if(options.pageParam.parentSourceParam){
				delete options.pageParam.parentSourceParam;
			}
			//参数来源处理
			if (!matrixVariable) {
				//正常参数处理
	...
	switch (_ajaxConfig.dataFormat) {
		...
		case 'custom':
			...
			if (_ajaxConfig.ajaxDataParamFormat) {
				...
				returnAjaxData = NetStarUtils.getFormatParameterJSON(format, objData, false);
			} else {
		...
		default:
			...
			if (isUseObject) {
				...
				returnAjaxData = NetStarUtils.getFormatParameterJSON(_ajaxConfig.data, objData);
...
// 处理组件的业务组件获取数据
nsServerTools.setObjectStateByFormatData = function(formatData, templateComponents){
	/**
	 * formatData : 格式化后的数据
	 * templateComponents : 模板组件
	 */
	for(var i=0; i<templateComponents.length; i++){
		var type = templateComponents[i].type;
		if(type != "vo"){
			continue;
		}
		var fields = templateComponents[i].field;
		var keyField = templateComponents[i].keyField;
		var _formatData = formatData;
		if(keyField != "root"){
			_formatData = formatData[keyField] ? formatData[keyField] : {};
		}
		for(var j=0; j<fields.length; j++){
			...
			switch(fieldType){
				case 'business':
					if(typeof(_formatData[fieldId]) == "object"){
						if( _formatData[fieldId].objectState == NSSAVEDATAFLAG.ADD ||  		// 新增
							_formatData[fieldId].objectState == NSSAVEDATAFLAG.DELETE ||    // 删除
							_formatData[fieldId].objectState == NSSAVEDATAFLAG.EDIT			// 修改
						){
							_formatData.objectState = _formatData.objectState === NSSAVEDATAFLAG.NULL ? NSSAVEDATAFLAG.EDIT : _formatData.objectState;
						}
					}
					break;
			}
