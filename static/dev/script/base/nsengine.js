var nsEngine = (function () {
        ...
        if (workitem.needSetOthersTransactor) {
            ...
            for (var index = 0; index < otherActivities.length; index++) {
                formArr.push({
                    ...
                    type: 'select',
                    subdata: JSON.parse(otherActivities[index].transactorsDef).map(transactorDef => {
                        return {
    ...
    OperationFun.prototype.findHandleRec = function (success, failure) {
        if (!this.info.workitemId || this.info.workitemId.length == 0) {
            nsalert('请指定要查看办理意见的工作项', 'warning');
            return;
        }
        var packageSuffix = '?packageSuffix=' + new Date().getTime();
        NetstarUI.labelpageVm.loadPage('/templateMindPages/pageConfig/1315230776517723122' + packageSuffix, '查看办理意见', true, {workitemId:this.info.workitemId});
    };
    /**
     * 输入办理意见
     * @param callBackFunc
